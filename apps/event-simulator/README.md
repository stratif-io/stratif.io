# @stratifio/seeder-studio

Interactive preset authoring tool for the `seeders/` simulator. Loads the
...
```bash
bun run --filter @stratifio/seeder-studio test:run
bun run --filter @stratifio/seeder-studio test:contract  # requires fixtures
```
...
`seeders/presets/<name>.yaml` and run `uv run seed --preset <name>`. The
...
with `seeders/simulator/`. If you change an axis's Python parameters,
