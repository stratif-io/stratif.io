...
describe('Segmented', () => {
  ...
  it('marks the active option with data-active', () => {
    ...
    expect(screen.getByRole('radio', { name: 'strong' })).toHaveAttribute('data-active', 'true')
    expect(screen.getByRole('radio', { name: 'weak' })).toHaveAttribute('data-active', 'false')
  })
  ...
  it('supports an optional tooltip per option', async () => {
    ...
    expect(await screen.findByRole('tooltip', { name: 'Alpha' })).toBeInTheDocument()
  })
})
