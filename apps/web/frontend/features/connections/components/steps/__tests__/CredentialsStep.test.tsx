...
describe('CredentialsStep', () => {
  ...
  it('calls onNext automatically after save + test succeeds', () => {
    vi.useFakeTimers()
    const onNext = vi.fn()
    let testMutateCallback: ((result: { ok: boolean }) => void) | undefined
    const testMutate = vi.fn((_id: string, opts?: { onSuccess?: (r: { ok: boolean }) => void }) => {
      testMutateCallback = opts?.onSuccess
    })
    vi.mocked(useTestConnection).mockReturnValue({
      mutate: testMutate,
      isPending: false,
    ...
    const updateMutate = vi.fn((_data: unknown, opts?: { onSuccess?: () => void }) => {
      updateSuccessCallback = opts?.onSuccess
    })
    vi.mocked(useUpdateConnection).mockReturnValue({
      mutate: updateMutate,
      isPending: false,
    ...
    // Trigger a credential change via form input
    const form = document.querySelector('form')!
    form.dispatchEvent(new Event('input', { bubbles: true }))
    vi.advanceTimersByTime(1000)

    // Simulate update completing successfully
    updateSuccessCallback?.()

    // Now test is triggered; simulate it succeeding
    testMutateCallback?.({ ok: true })

    expect(onNext).toHaveBeenCalled()
    vi.useRealTimers()
  })

  it('does NOT call onNext when save + test fails', () => {
    vi.useFakeTimers()
    const onNext = vi.fn()
    let testMutateCallback: ((result: { ok: boolean }) => void) | undefined
    const testMutate = vi.fn((_id: string, opts?: { onSuccess?: (r: { ok: boolean }) => void }) => {
