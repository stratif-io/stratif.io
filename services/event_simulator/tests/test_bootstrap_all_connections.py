...
from services.analytics.product_db.database import reset_engine
...
def _patch_db(db_url: str):
    reset_engine()
    with (
        patch("services.analytics.product_db.database.settings") as db_s,
        patch("services.analytics.services.crypto.settings") as crypto_s,
    ):
...
def _patch_backend():
    with patch(
        "services.event_simulator.bootstrap_all_connections.get_backend",
...
def test_bootstraps_only_enabled_backends(tmp_path):
    ...
    with _patch_db(url), _patch_backend():
        from services.event_simulator.bootstrap_all_connections import bootstrap
...
def test_runs_seeder_when_skip_seed_false(tmp_path):
    ...
    with (
        ...
        _patch_backend(),
        patch(
            "services.event_simulator.bootstrap_all_connections._run_seeder",
