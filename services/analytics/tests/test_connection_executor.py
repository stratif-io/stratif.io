...
from services.analytics.services.connection_executor import get_analytics_db
...
from services.analytics.services.connection_executor import (  # noqa: E402
    AnalyticsDatabase,
    _to_named_params,
)
...
def test_execute_raises_503_on_stale_databricks_connection():
    ...
    from services.analytics.backends.databricks import DatabricksBackend
...
class TestOpenAnalyticsDbIdentityFieldFilters:
    ...
    async def _open_db(self, schema_data: dict, filter_fields: list[dict]):
        ...
        from services.analytics.services.analytics_db import open_analytics_db
        ...
        with (
            patch(
                "services.analytics.services.crypto.decrypt_credentials",
...
def test_execute_reconnects_and_retries_on_stale_postgres_connection():
    ...
    with (
        patch("services.analytics.services.analytics_db._pool_evict") as mock_evict,
        patch(
            "services.analytics.services.analytics_db._pool_get",
...
async def test_open_analytics_db_raises_503_when_backend_open_fails():
    ...
    from services.analytics.backends.snowflake import SnowflakeBackend
    from services.analytics.services.analytics_db import open_analytics_db
