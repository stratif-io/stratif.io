...
class TestBrowsePoolUnpacking:
    ...
    def _patch_pool(self, db_conn):
        ...
        return patch(
            "backend.api.connections.browse._pool_get",
    ...
    def _patch_connection(self, db_type=DB_TYPE):
        ...
        return patch(
            "backend.api.connections.browse._get_connection_or_404",
            return_value=conn_model,
        )

    def _patch_credentials(self):
        return patch(
            "backend.api.connections.browse.decrypt_credentials",
    ...
    def test_browse_with_pool_does_not_pass_tuple_as_conn(self):
        ...
        with (
            ...
            patch("backend.api.connections.browse.get_backend") as mock_get_backend,
        ):
            backend = MagicMock()
            ...
            assert not isinstance(first_arg, tuple), (
                f"backend.browse() received a tuple instead of a connection: {first_arg!r}. "
