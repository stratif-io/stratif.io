...
export function EventPropertiesPanel({
    ...
    return (
      <>
        ...
        <p className="text-sm text-muted-foreground/60">
          Click an event to inspect its properties
        </p>
      </>
    );
  }
  ...
  const grouped = new Map<
    string,
    { cat: Category; entries: [string, unknown][] }
  >();
      ...
      {Array.from({ length: 6 }).map((_, i) => (
        <div key={i} className="flex gap-4">
          <div className="flex flex-col items-center">
            ...
            {i < 5 && (
              <div className="w-0.5 flex-1 bg-border/30 min-h-[2.5rem] my-1" />
            )}
          </div>
  ...
  return (
    ...
    >
      <div className="flex flex-col items-center flex-shrink-0">
        ...
        {!isLast && (
          <div className="w-0.5 flex-1 bg-border/50 min-h-[2rem] my-1" />
        )}
      </div>
      <div className={cn("flex-1 min-w-0 pt-0.5", isLast ? "pb-0" : "pb-5")}>
        <div className="flex items-center gap-1.5">
          <span
            className={cn("text-sm font-semibold leading-snug", color.text)}
          >
            {event.event_name}
          </span>
          {hasProps && !isSelected && (
