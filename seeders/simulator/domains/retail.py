"""Retail domain pack — session archetypes + event vocabulary."""

from __future__ import annotations

import random
import uuid
from datetime import datetime, timedelta

from seeders.simulator.domains._shared import event_tuple, pick_referrer
from seeders.simulator.protocols import DomainPack, SimulationState
from seeders.simulator.realism.inter_event import sample_inter_event_seconds

_ARCHETYPES = ("bounce", "browser", "researcher", "converter")


def _gap(rng: random.Random, kind: str = "normal") -> timedelta:
    return timedelta(seconds=sample_inter_event_seconds(rng, kind))


class RetailPack:
    name: str = "retail"
    events: tuple[str, ...] = (
        "PageView",
        "Search",
        "ProductView",
        "AddToCart",
        "Purchase",
        "StoreLocator",
        "InStorePickup",
        "LoyaltyRedemption",
    )
    supported_monetization: tuple[str, ...] = ("one_off_purchase",)

    def build_session(
        self,
        user: dict,
        session_start: datetime,
        archetype: str,
        state: SimulationState,
        rng: random.Random,
    ) -> list[tuple]:
        if archetype not in _ARCHETYPES:
            raise ValueError(f"unknown archetype {archetype!r}; valid: {_ARCHETYPES}")

        session_id = f"{user['id']}_{str(uuid.uuid4())[:6]}"
        referrer = pick_referrer(rng)
        t = session_start
        events: list[tuple] = []
        base_props = {
            "session_id": session_id,
            "is_returning_user": user["is_returning"],
        }

        # PageView — every archetype starts here
        events.append(event_tuple(rng, user, "PageView", t, dict(base_props), referrer))
        if archetype == "bounce":
            return events

        # Search
        t += _gap(rng, "normal")
        events.append(event_tuple(rng, user, "Search", t, dict(base_props), referrer))

        # ProductViews
        view_counts = {"browser": (1, 3), "researcher": (3, 6), "converter": (2, 5)}
        lo, hi = view_counts[archetype]
        for i in range(rng.randint(lo, hi)):
            t += _gap(rng, "normal" if i == 0 else "slow")
            props = dict(base_props)
            props["product_id"] = str(uuid.uuid4())[:8]
            events.append(event_tuple(rng, user, "ProductView", t, props, referrer))

        if archetype == "browser":
            # 30% chance of StoreLocator at end
            if rng.random() < 0.30:
                t += _gap(rng, "normal")
                events.append(
                    event_tuple(
                        rng, user, "StoreLocator", t, dict(base_props), referrer
                    )
                )
            return events

        if archetype == "researcher":
            # 50% chance of AddToCart — never converts
            if rng.random() < 0.50:
                t += _gap(rng, "normal")
                events.append(
                    event_tuple(rng, user, "AddToCart", t, dict(base_props), referrer)
                )
            return events

        # converter: AddToCart → Purchase → optional post-purchase event
        t += _gap(rng, "normal")
        events.append(
            event_tuple(rng, user, "AddToCart", t, dict(base_props), referrer)
        )

        t += _gap(rng, "rapid")
        purchase_props = dict(base_props)
        purchase_props["order_id"] = str(uuid.uuid4())[:12]
        purchase_props["currency"] = user["currency"]
        events.append(event_tuple(rng, user, "Purchase", t, purchase_props, referrer))

        # 40% chance of LoyaltyRedemption or InStorePickup
        if rng.random() < 0.40:
            t += _gap(rng, "rapid")
            post_event = rng.choice(["LoyaltyRedemption", "InStorePickup"])
            events.append(
                event_tuple(rng, user, post_event, t, dict(base_props), referrer)
            )

        user["completed_purchase"] = True
        return events


assert isinstance(RetailPack(), DomainPack)
