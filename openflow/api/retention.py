...
def get_retention(
    ...
    if db:
        ...
        cohort_where = (
            ("WHERE " + " AND ".join(cohort_clauses)) if cohort_clauses else ""
        )
        activity_where = (
        ...
        for cohort_date_str, info in cohorts.items():
            ...
            def unit_pct(
                u: int, _units_data=units_data, _cohort_size=cohort_size
            ) -> float:
            ...
            milestone_values = [
                unit_pct(m, units_data, cohort_size) for m in milestones
            ]
