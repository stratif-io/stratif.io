...
pub fn build_filter_values_query(_backend: &dyn AnyBackend, field: &str, start_date: &str, end_date: &str) -> String {
    let escaped_field = field.replace('\'', "''");
    format!(
        "SELECT DISTINCT {escaped_field} FROM events \
         WHERE timestamp >= '{start_date}' AND timestamp < '{end_date}' \
         AND {escaped_field} IS NOT NULL ORDER BY 1 LIMIT 100"
    )
}
